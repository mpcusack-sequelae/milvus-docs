...
<h1 id="Configure-Milvus-QueryNode-with-Local-Disk" class="common-anchor-header">Configure Milvus QueryNode with Local Disk
    <button data-href="#Configure-Milvus-QueryNode-with-Local-Disk" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>These machine types provide NVMe disk storage. You can use the <code>lsblk</code> command on the instances of these machine types to check if they have NVMe disk storage. If they do, you can proceed to the next step.</p>
<pre><code class="language-bash">$ lsblk | grep nvme
nvme0n1     259:0    0 250.0G  0 disk 
nvme1n1     259:1    0 250.0G  0 disk 
<button class="copy-code-btn"></button></code></pre>
<h2 id="Configure-Kubernetes-to-use-local-disk" class="common-anchor-header">Configure Kubernetes to use local disk
    <button data-href="#Configure-Kubernetes-to-use-local-disk" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>In the above example, we assume that the NVMe disk is <code>/dev/nvme1n1</code>. You need to modify the script to match your specific configuration.</p>
</div>
...
<pre><code class="language-bash">gcloud container node-pools create <span class="hljs-variable">${POOL_NAME}</span> \
...
<pre><code class="language-bash">mdadm -Cv /dev/md0 -l0 -n2 /dev/nvme0n1 /dev/nvme1n1
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>In the above example, we assume that the NVMe disks are <code>/dev/nvme0n1</code> and <code>/dev/nvme1n1</code>. You need to modify the script to match your specific configuration.</p>
</div>
...
<button class="copy-code-btn"></button></code></pre>
<div class="alert note">
<p>In the above example, we assume that the NVMe disk is <code>/dev/nvme0n1</code>. You need to modify the script to match your specific configuration.</p>
</div>
...
<p>Ensure that your NVMe disk is properly mounted on your host machine. You can mount it to a directory of your choice. For instance, if you mount it to <code>/mnt/nvme</code>, make sure it is correctly set up and you can see the disk available at <code>/mnt/nvme</code> by running <code>lsblk</code> or <code>df -h</code>.</p></li>
...
<pre><code class="language-bash"><span class="hljs-built_in">sudo</span> <span class="hljs-built_in">mkdir</span> -p /mnt/nvme/containerd /mnt/nvme/containerd/state
<span class="hljs-built_in">sudo</span> vim /etc/containerd/config.toml
<button class="copy-code-btn"></button></code></pre>
<p>Locate the <code>[plugins.&quot;io.containerd.grpc.v1.cri&quot;.containerd]</code> section, and modify the <code>snapshotter</code> and <code>root</code> settings as follows：</p>
<pre><code class="language-toml">[<span class="hljs-meta">plugins.<span class="hljs-string">&quot;io.containerd.grpc.v1.cri&quot;</span>.containerd</span>]
...
<pre><code class="language-bash"><span class="hljs-built_in">sudo</span> systemctl restart containerd
<button class="copy-code-btn"></button></code></pre></li>
</ul>
<h2 id="Verify-disk-performance" class="common-anchor-header">Verify disk performance
    <button data-href="#Verify-disk-performance" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
...
<pre><code class="language-bash">kubectl create -f ubuntu.yaml
<button class="copy-code-btn"></button></code></pre>
<p>The <code>ubuntu.yaml</code> file is as follows:</p>
...
<pre><code class="language-bash"><span class="hljs-comment"># enter the container</span>
...
<pre><code class="language-bash">Jobs: <span class="hljs-number">128</span> (f=<span class="hljs-number">128</span>): [r(<span class="hljs-number">128</span>)][<span class="hljs-number">100.0</span>%][r=1458MiB/s][r=373k IOPS][eta 00m:00s]
...
<h2 id="Deploy-Milvus-Distributed" class="common-anchor-header">Deploy Milvus Distributed
    <button data-href="#Deploy-Milvus-Distributed" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
