...
<h2 id="Installing-the-requirements" class="common-anchor-header">Installing the requirements
    <button data-href="#Installing-the-requirements" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>For this example, we are going to be using <code>pymilvus</code> to connect to use Milvus, <code>torch</code> for running the embedding model, <code>torchvision</code> for the actual model and preprocessing, <code>gdown</code> to download the example dataset and <code>tqdm</code> for loading bars.</p>
<pre><code class="language-shell">pip install pymilvus torch gdown torchvision tqdm
...
<h2 id="Global-Arguments" class="common-anchor-header">Global Arguments
    <button data-href="#Global-Arguments" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
...
<pre><code class="language-python"><span class="hljs-comment"># Create an AutoIndex index for collection</span>
index_params = {
...
<h2 id="Inserting-the-data" class="common-anchor-header">Inserting the data
    ...
    </button></h2><p>For this example, we are going to use the ResNet50 model provided by <code>torch</code> and its model hub. To get the embeddings, we are taking off the final classification layer, which results in the model giving us embeddings of 2048 dimensions. All the vision models found on <code>torch</code> use the same preprocessing that we have included here.</p>
...
finish = time.time()
<button class="copy-code-btn"></button></code></pre>
<pre><code class="language-python"><span class="hljs-comment"># Show the image results</span>
...
<p>
  <span class="img-wrapper">
    <img src="/docs/v2.4.x/assets/integrate_with_pytorch.png" alt="Image search output" class="doc-image" id="image-search-output" />
