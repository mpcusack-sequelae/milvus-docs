...
<p>{{tab}}</p>
<h1 id="Upgrade-Milvus-Cluster-with-Docker-Compose" class="common-anchor-header">Upgrade Milvus Cluster with Docker Compose
    <button data-href="#Upgrade-Milvus-Cluster-with-Docker-Compose" class="anchor-icon">
      <svg
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><p>Change the Milvus image tags in <code>docker-compose.yaml</code>.</p>
...
<button class="copy-code-btn"></button></code></pre></li>
<li><p>Prepare the configuration file <code>migrate.yaml</code> for meta migration.</p>
...
<pre><code>Update the milvus image tag in the docker-compose.yaml
docker compose down
docker compose up -d
<button class="copy-code-btn"></button></code></pre></li>
</ol>
